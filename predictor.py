...
def predict_vm(ecs_lines, input_lines):
    ...
    begin_date = datetime.datetime.strptime((input_lines[-2].strip('\r\n')),"%Y-%m-%d %H:%M:%S")
    end_date = datetime.datetime.strptime((input_lines[-1].strip('\r\n')),"%Y-%m-%d %H:%M:%S")
    ...
    for i in range(len(server_property.ser_name)):
        ...
        for j in range(len(flavor_property.name)):
            if(server_property.flavor[i][j] != 0):
                temp_list_string = temp_list_string + (' ' + str(flavor_property.name[j]) + ' ' + str(str(server_property.flavor[i][j])))
